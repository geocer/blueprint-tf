locals {
  ...
  # === CORREÇÃO AQUI: Garante que ecs_services_input seja um mapa de objetos ===
  # Isso força o Terraform a reavaliar a estrutura de ecs_services
  ecs_services_input = {
    for k, v in var.ecs_services : k => v
  }
  # ==============================================================================

  # Mapeamento para criar apenas o ALB (sem listeners)
  ...
  # ====================================================================
  # Como o 'flat_alb_listeners' é construído para 'for_each'
  # Usando a sintaxe correta para for aninhado que produz um objeto
  # ====================================================================
  ...
  ecs_service_configs = { for service_name, service_attrs in local.ecs_services_input : service_name => {
    ...
    # Usando 'lookup' para robustez
    task_exec_role_additional_iam_policy_arns = lookup(service_attrs, "additional_task_role_policy_arns", [])
  }}
